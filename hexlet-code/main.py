def run_all():
    ...
    def visits_API(api_url, date_begin, date_end):
        response = requests.get(f"{api_url}/visits?begin={date_begin}&end={date_end}")
        data = response.json()
        df_visits = pd.DataFrame(data)
        return df_visits
    ...
    # Вызов функции с передачей значений переменных
    df_visits = visits_API(api_url, date_begin, date_end)
    ...
    def registrations_API(api_url, date_begin, date_end):
        response = requests.get(f"{api_url}/registrations?begin={date_begin}&end={date_end}")
        data = response.json()
        df_registrations = pd.DataFrame(data)
        return df_registrations
    ...
    # Вызов функции с передачей значений переменных
    df_registrations = registrations_API(api_url, date_begin, date_end)
    ...
    def visit_grouped(api_url, date_begin, date_end):
        response = requests.get(f"{api_url}/visits?begin={date_begin}&end={date_end}")
        data = response.json()
        df_visits = pd.DataFrame(data)
        df_visits['datetime'] = pd.to_datetime(df_visits['datetime'])
    ...
    # Вызов функции с передачей значений переменных
    df_visit_grouped = visit_grouped(api_url, date_begin, date_end)
    ...
    def registrations_grouped(api_url, date_begin, date_end):
        response = requests.get(f"{api_url}/registrations?begin={date_begin}&end={date_end}")
        data = response.json()
        df_registrations = pd.DataFrame(data)
        df_registrations['date_group'] = pd.to_datetime(df_registrations['datetime']).dt.strftime('%Y-%m-%d')
    ...
    # Вызов функции с передачей значений переменных
    df_registrations_grouped = registrations_grouped(api_url, date_begin, date_end)
    ...
    API_URL = os.getenv('API_URL')
    DATE_BEGIN = pd.to_datetime(os.getenv('DATE_BEGIN'))
    DATE_END = pd.to_datetime(os.getenv('DATE_END'))
